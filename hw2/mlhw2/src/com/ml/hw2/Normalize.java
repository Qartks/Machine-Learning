	...
	static void normalizeDataSandS(DataSet trainSet) throws Exception {
		...
		for(int i = 0; i< noOfFeatures; i++){
			...
			for(Data d: trainSet.getData()){
				...
				if(d.getFeatureValue(i) > featureMax[i]){
					featureMax[i] = d.getFeatureValue(i);
				}
				if(d.getFeatureValue(i) < featureMin[i]){
					featureMin[i] = d.getFeatureValue(i);
				}
