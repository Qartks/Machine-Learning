...
public class ECOCImplmentation {
	...
	public void generateErrorTable() {
		...
		while(list.size() != noOfClassifiers){
			...
			String s = padLeft(Integer.toBinaryString(num), noOfLabels).replace(' ', '0');
...
//				}
	...
	public void train(DataSet trainData, DataSet testData, int adaClassifiers, boolean optimal, boolean stats) throws Exception {
		...
		for(int i = 0; i < noOfClassifiers; i++){
			...
			AdaBoostImplementation ada = new AdaBoostImplementation(adaClassifiers, optimal);
			ada.train(trainData, testData, stats);
			classifiers.add(ada);
		}
...
//				System.out.print(ada.predictValue(d) + " " + d.labelValue);
				res[count] = ada.getPredictedLabel(d);
				count++;
			}
