	...
	public static void learn(DataSet dataSet, double ilambda, boolean logistic,
		...
		while(iteration < 4000){
			...
			for(int row = 0; row < x.numRows(); row++) {
				...
				for(int col=0; col < x.numCols(); col++) {
					double newWeightValue = updateWeight(w.get(col,0), predictedValue, y.get(row, 0), x.get(row, col),logistic);
...
//		}
	...
	private static double updateWeight(double old, double predictedValue, double y, double x, boolean logistic) {
		double newW  = 0;
		if(logistic){
			newW = old + lambda * (y - predictedValue) * x; 
		} else {
			newW = old - lambda * (predictedValue - y) * x;
		}
		return newW;
	}
